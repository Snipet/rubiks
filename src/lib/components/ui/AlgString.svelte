...
<script lang="ts">
	...
	const parsed = $derived.by((): Move[] | null => {
		try {
			return parseAlg(alg);
		} catch {
			return null;
	...
	/** Respect the reader's preference for `Rw` versus `r`. */
	...
	const moves = $derived(parsed ?? []);
	const length = $derived(parsed ? htmLength(parsed) : 0);
</script>
...
{:else}
	<span
		...
		aria-label={formatAlg(moves)}
	>
		{#each moves as move, i (i)}
			{@const token = display(move)}
